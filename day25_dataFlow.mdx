...
- we know that we can pass data from a `parent component` ---> `child component` using *props drilling*
...
- But it is also possible to pass data from a `child component` ---> `parent component` using props: 
    ...
    - using *callback function* (see Ex2_stateLifting). This process is called `state lifting`.
